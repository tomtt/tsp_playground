RSpec.describe StarterGem::Shell do
  ...
  it "exits without replacing if no argument is passed" do
    ...
    expect(-> { ::StarterGem::Shell.run(argv, out: out_double, err: err_double) }).to raise_error SystemExit
  end

  it "does something if there is one argument" do
    argv = ["cookies"]
    ::StarterGem::Shell.run(argv, out: out_double, err: err_double) do |options|
  ...
  it "raises an error if no block is given" do
    argv = ['blue']
    expect(-> { ::StarterGem::Shell.run(argv, out: out_double, err: err_double) }).to raise_error LocalJumpError
  end
end
